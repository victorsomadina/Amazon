...
SELECT DISTINCT COUNT(COUNTRY_NAME) TOTAL_COUNTRIES FROM REGIONS;

SELECT DISTINCT R.COUNTRY_NAME, REGION, FOREST_AREA_SQKM, TOTAL_AREA_SQ_MI, TOTAL_AREA_SQ_MI- FOREST_AREA_SQKM AS DEFORESTED_AREA
FROM FOREST_AREA F
JOIN LAND_AREA L ON F.COUNTRY_CODE = L.COUNTRY_CODE
RIGHT JOIN REGIONS R ON F.COUNTRY_CODE = R.COUNTRY_CODE
ORDER BY R.COUNTRY_NAME;

-- QUESTION TWO

SELECT INCOME_GROUP, COUNT(TOTAL_AREA_SQ_MI) AS TOTAL_AREA FROM LAND_AREA L JOIN REGIONS R ON L.COUNTRY_CODE = R.COUNTRY_CODE 
WHERE TOTAL_AREA_SQ_MI BETWEEN 75000 AND 150000
GROUP BY INCOME_GROUP;

SELECT COUNT (*) AS TOTAL_NUMBER_OF_COUNTRIES FROM (SELECT L.COUNTRY_NAME, INCOME_GROUP, COUNT(TOTAL_AREA_SQ_MI) OVER(PARTITION BY INCOME_GROUP) AS TOTAL_AREA
